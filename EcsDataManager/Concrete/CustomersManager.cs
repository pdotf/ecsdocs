...
namespace EcsDataManager.Concrete
{
    public class CustomersManager : ICustomersManager
    {
        ...
        public Task<CustomerView> GetCustomerJoinjwithDeviceById(int Id)
        {
            var customer = Task.FromResult(_dapperManager.Get<CustomerView>($@"select a.id as Id,CustomerName,Tel,Mobile,OwnerTeam,ServiceType,ServiceTopology,AccountManager,IpHQ,AAAGroup,IpTunnel,WanIpRange,LanIpRange,VRF,VpnToolsName,APN,Comment,CustomerId,RadioMainName,RadioMainIp,RadioMainModel,RadioMainMetroSite,RadioBackupName,RadioBackupIp,RadioBackupModel,RadioBackupMetroSite,BackupActivationIsAuto
             from [Customers] a left join Devices b on a.id=b.CustomerId where a.ID= {Id}", null,
                     commandType: CommandType.Text));
            return customer;
        }
